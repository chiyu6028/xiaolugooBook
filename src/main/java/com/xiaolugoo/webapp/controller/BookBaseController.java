...
public class BookBaseController {
    ...
    public String insertSelective(BookBase bookBase) throws JsonProcessingException {
        ...
        try {
            ...
            if (res > 0){
                //获取完整数据
                bookBase = bookBaseService.selectByPrimaryKey(bookBase.getBookId());
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "bookStatus", value = "书本状态", paramType = "query", dataType = "String")
    })
    ...
    public String selectAll(@PathParam("bookId") Integer bookId,
        ...
        logger.debug("批量查询书本");
        ResultData resultData = null;
        List<BookBase> bookBaseList = null;
        ...
        try {
            bookBaseList = bookBaseService.selectAll(bookMap);
    ...
    @ApiOperation(value = "搜索", notes = "一切都在搜索")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "key", value = "搜索关键字", required = true, paramType = "query", dataType = "String"),
            @ApiImplicitParam(name = "order", value = "是否最新发布排序", paramType = "query", dataType = "boolean"),
            @ApiImplicitParam(name = "bookStatus", value = "书本状态", paramType = "query", dataType = "String"),
            @ApiImplicitParam(name = "pageOffset", value = "分页", paramType = "query", dataType = "Integer"),
            @ApiImplicitParam(name = "pageSize", value = "分页大小", paramType = "query", dataType = "Integer")
    })
    @RequestMapping(value = "/search", method = {RequestMethod.GET, RequestMethod.POST})
    public String search(@PathParam("key") String key,
                         @PathParam("order") Boolean order,
                         @PathParam("bookStatus") String bookStatus,
                         @PathParam("pageOffset") Integer pageOffset,
                         @PathParam("pageSize") Integer pageSize) throws JsonProcessingException {
        logger.debug("批量搜索书本");
        ResultData resultData = null;
        List<BookBase> bookBaseList = null;
        HashMap bookMap = new HashMap();
        bookMap.put("key", key);
        bookMap.put("order", order);
        bookMap.put("bookStatus", bookStatus);
        bookMap.put("pageOffset", pageOffset);
        bookMap.put("pageSize", pageSize);
        //1.先从elasticsearch获取数据，如果能获取直接返回，如果获取失败，送数据库查询
        bookBaseList = bookBaseService.findBookAllByEs(bookMap);
